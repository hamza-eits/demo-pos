<tbody id="card-invoice-list">
    @foreach ($data->where('mode','card') as $value)
        <tr>
            <td class="text-start">{{ $loop->iteration }}</td>
            <td class="text-start">{{ date('d-m-Y h:i:s A', strtotime($value->created_at)) }}</td>
            {{-- <td class="text-start">{{ $value->mode }}</td> --}}
    ...
    @endforeach
        <tr>
            <th colspan="6" class="text-start">Total Card Sales</th>
            <th class="text-center">{{ $cardSale }}</th>
        </tr>
</tbody>