...
<tbody id="invoice-list">
    @foreach ($data->unique('invoice_master_id') as $value)
        <tr>
            ...
            <td class="text-end">{{ $value->invoiceMaster->grand_total }}</td>
        
        </tr>   
    @endforeach
        <tr>
            <th colspan="6" class="text-start">Total Invoice Sales</th>
            <th class="text-end">{{ $data->unique('invoice_master_id')->sum('amount') }}</th>
        </tr>
</tbody>
