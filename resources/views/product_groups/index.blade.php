                ...
                <div class="row">
                    <div class="col-12">
                        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                            <h3 class="mb-sm-0 font-size-18">All Product Groups</h3>
    ...
    <script>
        ...
        $(document).ready(function() {
            table = $('#table').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('product-group.index') }}",
                columns: [
                    { data: 'id'},
                    { data: 'name' },
                    {
        ...
        function editRecord(id) {
            $.get("{{ route('product-group.edit', ':id') }}".replace(':id', id), function(response) {
        ...
        function deleteRecord(id) {
            if (confirm("Are you sure you want to delete?")) {
                $.ajax({
                    type: 'DELETE',
                    url: "{{ route('product-group.destroy', ':id') }}".replace(':id', id),
                    data: {
